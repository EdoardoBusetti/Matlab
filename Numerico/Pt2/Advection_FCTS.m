...
% du/dx -> We do taylor in xn + dx and xn - dx always at time j, second order and we
% subtract them
% du/dt -> We do taylor in u_(n , j+1) 1st order.
% We then use them both to get:
% u_(n , j+1) = u_(n,j) + c * dt/2dx * (u_(n+1,j) - u_(n-1,j))  -> Valid
% for all n = 1, .... N-1 -->  for n = 0 and n = N we use boundary
% conditions
